...
public open class Converter() {
    ...
    class object {
        ...
        public val PRIMITIVE_TYPE_CONVERSIONS: Map<String, String> = ImmutableMap.builder<String, String>()
                ...
                ?.put("int", INT.getName())
                ?.put("long", LONG.getName())
                ?.put("float", FLOAT.getName())
                ?.put("double", DOUBLE.getName())
                ?.put("char", CHAR.getName())
                ?.put(JAVA_LANG_BYTE, BYTE.getName())
                ?.put(JAVA_LANG_SHORT, SHORT.getName())
                ?.put(JAVA_LANG_INTEGER, INT.getName())
                ?.put(JAVA_LANG_LONG, LONG.getName())
                ?.put(JAVA_LANG_FLOAT, FLOAT.getName())
                ?.put(JAVA_LANG_DOUBLE, DOUBLE.getName())
                ?.put(JAVA_LANG_CHARACTER, CHAR.getName())
                ?.build()!!
